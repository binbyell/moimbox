...
class CommentWidget extends StatefulWidget {
  ...
  CommentWidget({Key? key, required this.commentItem, required this.noticeItem, required this.meetingId}): super(key: key);

  @override
  _tempPage createState() => _tempPage();
}

class _tempPage extends State<CommentWidget> {
                      ...
                      if(widget.commentItem.onerUid == metaMyProfileItem.uid){
                        return [
                          ...
                          PopupMenuItem(
                            ...
                            value: 2,
                            onTap: (){
                              print("widget.noticeItem.commentList : ${widget.noticeItem.commentList}");
                              widget.noticeItem.commentList.remove(
                                  widget.noticeItem.commentList.where((element) => element['TIME'] == widget.commentItem.time && element['ONER_UID'] == widget.commentItem.onerUid).first
                              );
                              setState((){
                                visibleTemp = false;
                              });
